...
import { crearPrestamos, editarEstadoPrestamos } from "../Services/Prestamos";
import { crearPagos } from "../Services/Pagos";
import { useNavigate, useParams } from "react-router-dom";

const obtenerPrestamos = async (idCliente) => {
  try {
    const response = await axios.get(
      "http://localhost:8080/prestamos/usuario/" + idCliente
    );
...
const ListaPrestamosUsuario = () => {
  ...
  const [alertaPrestamo, setAlertaPrestamo] = useState(null);
  ...
  useEffect(() => {
    const cargarPrestamos = async () => {
      try {
        const datosPrestamos = await obtenerPrestamos(idCliente);
  ...
  const abrirModalPrestamo = () => {
    ...
    setAlertaPrestamo(null);
  };
  const obtenerFechaActualCorta = () => {
    const hoy = new Date();
    const pad = (n) => n.toString().padStart(2, "0");
    return `${hoy.getFullYear()}-${pad(hoy.getMonth() + 1)}-${pad(
      hoy.getDate()
    )}`;
  };
  ...
  const guardarModalPrestamo = async () => {
    const monto = parseFloat(montoPrestamo);
    const plazo = parseInt(plazoPrestamo);

    if (isNaN(monto) || monto <= 0 || isNaN(plazo) || plazo <= 0) {
      setAlertaPrestamo(
        "Por favor ingrese valores válidos para monto y plazo."
      );
      return;
    }
    const data = {
  ...
  return (
    ...
    >
      ...
      <Dialog
        open={modalPrestamoAbierto}
        onClose={cerrarModalPrestamo}
        fullWidth
        maxWidth="sm"
      >
        <DialogTitle>Formulario de Préstamo</DialogTitle>
        <DialogContent>
          {alertaPrestamo && (
        ...
        <DialogActions>
          <Button onClick={cerrarModalPrestamo} color="secondary">
            Cancelar
          </Button>
          <Button
            onClick={guardarModalPrestamo}
            color="primary"
            variant="contained"
          >
            Guardar
...
export default ListaPrestamosUsuario;
